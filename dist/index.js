...
var RegionReferenceError = class extends ReferenceError {
  constructor(message = "Oups! Ce n'est pas une r\xE9gion valide !") {
    super(message);
    this.name = "RegionReferenceError";
  }
};
var DepartmentReferenceError = class extends ReferenceError {
...
// src/libs/utils.ts
var lowerCase = (value) => (value == null ? undefined : value.toLowerCase().trim()) ?? "";
var findItem = (items, value) => {
  const region = items.find((region2) => lowerCase(region2.nom) === lowerCase(value));
  if (!region) throw new RegionReferenceError();
  return region;
};
var departmentItem = (items, value) => {
...
// src/index.ts
var sn_data = senegal_default;
var rg_data = regions_default;
var dp_data = departments_default;
var Galsenify = class {
  // Données brutes
  sn_data = sn_data;
  rg_data = rg_data;
...
var galsenify = new Galsenify();
var index_default = galsenify;
